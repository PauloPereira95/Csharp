...
namespace ConsoleApp1
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"{"Produto",-80}{"Marca",-20}{"Categoria",-15}" +
                $"{"Peco S/Iva",-25}{"Preço C/IVA",-25}{"Stock",-10}" +
                $"{"Data de Inicio de Venda",30}\n");
            Console.ResetColor();
            foreach (var item in produtos)
            {
                Console.WriteLine($"{item.Nome, -80}{item.Marca,-20}{item.Categoria,-15}" +
        ...
        static void exe_06()
        {
            ...
            Console.ResetColor();
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"{"Produto",-80}{"Preço Final",-15}{"Marca",-20}{"Categoria",-20}\n");
            Console.ResetColor();
            foreach (var item in produtos.OrderBy(p => p.PrecoFinal))
            {
                Console.WriteLine($"{item.Nome,-80}{item.PrecoFinal,-15:C}{item.Marca,-20}{item.Categoria,-20}");
            }
            ...
            Console.ResetColor();
            Console.WriteLine($"Preço Médio Perifericos -> {Math.Round(produtos.Where(c => c.Categoria == "Periféricos").Average(p => p.PrecoFinal),2):C}");
            ...
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"{"Produto",-80}{"Marca",-20}{"Categoria",-20}\n");
            Console.ResetColor();
            foreach (var item in produtos.Where(c => c.Nome.Contains("SSD")))
            {
                Console.WriteLine($"{item.Nome,-80}{item.Marca,-20}{item.Categoria,-20}");
            }
            // 3.4. Indicar o total de produtos (Count) da marca Asus;
            Console.WriteLine();
            Console.WriteLine("-------------- Total Produtos Asus --------");
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"{"Produto",-80}{"Marca",-20}{"Categoria",-20}{"Preço Final",-20}{"IVA",-20}\n");
            Console.ResetColor();
            foreach (var item in produtos.Where(c => c.Marca =="Asus"))
            {
                Console.WriteLine($"{item.Nome,-80}{item.Marca,-20}{item.Categoria,-20}{item.PrecoFinal,-20:C}{item.Iva1,-20}");
            }
            ...
            Console.ResetColor();
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine($"{"Produto",-80}{"Data Inicio de Venda",-30}{"Marca",-20}{"Categoria",-30}\n");
            Console.ResetColor();
            foreach (var item in produtos.Where(f => f.DataInicioVenda.Year == 2019 || f.DataInicioVenda.Year == 2020))
            {
                Console.WriteLine($"{item.Nome,-80}{item.DataInicioVenda.ToShortDateString(),-20}{item.Marca,-20}{item.Categoria,-20}");
            }
